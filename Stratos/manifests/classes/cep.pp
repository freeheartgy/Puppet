class stratos::cep ( $version, 
	...
	$carbon_home		= "${target}/wso2${service_code}-${stratos_version}"
        ...
        $service_templates      = ["conf/registry.xml","conf/carbon.xml","conf/security/authenticators.xml",
				   "deployment/server/webapps/STRATOS_ROOT/index.html", "conf/etc/cassandra-auth.xml", "conf/etc/cassandra-component.xml",
				   "conf/user-mgt.xml", "conf/tomcat/catalina-server.xml"]

	$common_templates       = ["conf/etc/cache.xml","conf/multitenancy/cloud-services-desc.xml","conf/datasources/master-datasources.xml",
			   "conf/identity.xml","conf/etc/logging-config.xml","conf/log4j.properties",  "conf/multitenancy/stratos.xml"]
